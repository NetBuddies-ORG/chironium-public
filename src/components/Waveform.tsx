...
export default function Waveform({ 
  ...
  useEffect(() => {
    ...
    const waveformBg = computedStyle.getPropertyValue('--waveform-bg').trim() || '#0D1117';
    const waveformColor = computedStyle.getPropertyValue('--waveform-color').trim() || '#58a6ff';
    const playheadColor = computedStyle.getPropertyValue('--playhead-color').trim() || '#00C2FF';
    const gridColor = computedStyle.getPropertyValue('--grid-color').trim() || '#30363d';
  ...
  return (
    <div className="relative">
      <canvas
        ...
        width={width}
        height={height}
        className="cursor-pointer rounded border border-[var(--app-border)]"
        onClick={handleClick}
      />
    </div>
  );
